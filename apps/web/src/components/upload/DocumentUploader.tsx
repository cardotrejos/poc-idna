...
const CATEGORIES = ["certificate", "academic", "award", "reference", "other"] as const;
...
export function DocumentUploader({
  ...
  onUploaded?: () => void;
  studentUserId?: string; // optional: coach/admin can upload for a student
}) {
  ...
  const upload = useCallback(async () => {
    ...
    try {
      for (const item of selected) {
        ...
        if (studentUserId) fd.append("studentUserId", studentUserId);
        const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/uploads/documents`, {
          method: "POST",
          body: fd,
          credentials: "include",
        });
      ...
      toast.success(`Uploaded ${selected.length} document(s)`);
  ...
  }, [selected, category, onUploaded, studentUserId]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        onDrop={onDrop}
        aria-label="Upload document (JPG, PNG or PDF)"
        className={`border-2 border-dashed rounded p-4 text-center cursor-pointer ${dragOver ? "bg-blue-50" : ""} ${busy ? "opacity-60" : ""}`}
      >
        <p>Drag and drop document(s) here, or click to select.</p>
        ...
        <div className="mt-3 space-y-2" aria-live="polite">
          ...
          <div className="flex gap-2">
            <button type="button" className="border rounded px-3 py-2" onClick={upload} disabled={busy}>
              {busy ? `Uploading ${selected.length}…` : `Upload ${selected.length} document(s)`}
            </button>
            <button
              type="button"
