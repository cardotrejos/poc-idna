...
export const googleExtractor: AssessmentExtractor = {
  async extract({ bytes, mimeType, typeSlug }): Promise<ExtractedResult> {
    ...
    try {
      ...
      if (shouldLogRaw) {
        console.info("[ai][google] object", { typeSlug, parsed });
      }
      ...
      if (shouldLogRaw) {
        console.warn("[ai][google] structured output failed", {
          typeSlug,
      ...
      try {
        ...
        if (plaintext.length > 0) {
          ...
          if (shouldLogRaw) {
            console.info("[ai][google] secondary extraction", { keys: Object.keys(obj3), obj3 });
          }
        ...
        if (shouldLogRaw) {
          console.error("[ai][google] secondary attempt failed", { error: (err2 as Error)?.message });
        }
      }
